...
**사전 계산 데이터 안내** — 아래 수치는 시스템이 이미 계산해서 제공합니다. 재계산하지 말고 해석과 맥락 판단에 집중하십시오.
- 기술 진입점수(0~100): RSI·BB·MA배열·MACD·거래량·52주위치 복합 채점
- CAN SLIM 종합점수(0~100): C/A/N/S/L/I/M 팩터 각각 점수화 후 정규화
- ATR 기반 손절가(ATR×1.5)·1차익절(ATR×2.0)·2차익절(ATR×3.5) — 이미 $ 수치로 제공
- R/R 비율: (목표가 - 현재가) / (현재가 - 손절가) — 이미 계산됨
- 52주 고저 위치(%): (현재가 - 52주저) / (52주고 - 52주저) × 100
...
🧠 전문가 C (Behavioral Analyst): '내 현재 심리 상태'와 '오늘 하고 싶은 행동'을 아래 8가지 심리 유형으로 진단하십시오.
① 익절 불안 → 처분 효과 편향, 조기 이익 실현 충동 경고
② 손실 회피(본전 심리) → 매몰 비용 오류, 준거점 의존성 지적
③ FOMO(벼락거지 공포) → 군중 심리 추종 여부, 고점 매수 리스크 경고
④ 비자발적 장기투자(물린 상태) → 회피적 현실 부정 가능성 진단
⑤ 이성적 분할 매매 → 전략 일관성 확인, 과신 여부만 체크
⑥ 단순 관망 → 기회비용 및 FOMO 잠재 가능성 점검
⑦ 손절 고민 → 손실 회피 편향 vs 합리적 리스크 관리 구분, 손절선 이미 이탈 시 즉각 손절 촉구
⑧ 목표가 임박 익절 타이밍 → 탐욕 편향(목표 이동) 경고, 부분 익절 원칙 제시
CAN SLIM M(시장 방향)에 역행하는 매매인지 체크하고, 행동 계획이 현재 심리 편향과 일치하는지 날카롭게 비판하십시오.

⚖️ 의장 (Moderator): A·B·C 의견과 CAN SLIM 7개 팩터를 종합해 최종 판결을 내립니다. 제공된 CAN SLIM 충족 팩터 수를 그대로 인용하고, 과거 예측 대비 현재 주가로 승률을 채점하십시오.

**분석 원칙**
- VPA: 가격 상승 시 거래량 감소(매수세 약화), 가격 하락 시 거래량 급증(투매) 반드시 확인.
- CAN SLIM N 조건: 제공된 N 팩터 값 기준으로, 거래량 미동반 돌파 시 페이크아웃 경고.
...
- 손익비 판단: 제공된 R/R 값 사용. R/R < 1:2면 "매매 보류" 선언.
...
[C. 추세&지표] 이평선 배열·제공된 RSI/Stochastic 값 해석
[D. 옵션 수급] Max Pain vs 현재가 괴리·만기별 PCR·단기 자석 가격대

### 3.5 📈 CAN SLIM 팩터 분석
제공된 점수 데이터를 근거로 7개 팩터 각각을 ✅(충족)/❌(미충족)/➖(데이터 없음)으로 평가하고 투자 판단 근거 서술.
...
### 5. ⚖️ 종합 분석
추세 판단 + 제공된 지표 해석 + 매수존(1차·2차) · 저항라인(1차·2차)

### 6. 🦁 의장의 최종 전략
> 핵심 결론 (CAN SLIM 충족 수 X/7)
[시나리오 A - 적립] 전략·매집 구간·목표
[시나리오 B - 리스크 관리] 경고 조건·최후 방어선
[R/R 판단] 제공된 손절가·목표가·R/R 수치 인용 → 진입 승인 여부
...
"""
    ...
    risk = cur - data['stop_loss']
    rr1  = round((data['target1'] - cur) / risk, 2) if risk > 0 else 0
    rr2  = round((data['target2'] - cur) / risk, 2) if risk > 0 else 0
    rr_verdict = "✅ 진입 승인" if rr1 >= 2 else "⛔ 진입 보류"

    user_prompt = f"""
### 📊 [분석 데이터]
- 종목: {data['ticker']} (현재가: ${cur:.2f} / 기준일: {data['last_date']})
...
- 📈 CAN SLIM 종합: {cs_total}점 → {cs_grade}
- 📐 ATR 기반 가격대: 손절 ${data['stop_loss']:.2f} / 1차익절 ${data['target1']:.2f} / 2차익절 ${data['target2']:.2f}
- 📐 R/R: 1차 1:{rr1} / 2차 1:{rr2} → {rr_verdict}
...
※ Audit 기준: 위 기억의 예측을 현재가 ${cur:.2f} 기준으로 냉정하게 채점하십시오.
"""

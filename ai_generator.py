...
- 옵션 수급: Max Pain(옵션 시장이 만기 때까지 끌어당기는 자석 가격)과 만기별 PCR을 Section 3[D]에 반드시 분석하십시오. 데이터가 "수집 불가"인 경우에도 해당 섹션을 작성하고 데이터 부재 사실을 명시하십시오.
...
- 기록 없음 → 🆕 첫 분석 — 검증 불가 (승자 판정 생략, 이 섹션 3줄 이내로 마감)
- 기록 1건 → 이전 목표가·손절가 대비 현재가로 단방향 채점
- 기록 2-3건 → 가장 최근 기록 기준 양방향 검증 (A: 단기 목표 달성 여부 / B: 장기 추세 유지 여부)
- 승자 기준: 목표가에 먼저 도달한 전문가 🏆 / 둘 다 미달이거나 동시 달성 → 🤝무승부

### 3. 📊 기술적 정밀 분석
[A. 캔들&차트 패턴] 일봉·주봉 패턴명과 해석
[B. 엘리엇 파동&피보나치] 현재 파동 위치(날짜·가격 포함)·지지 1차·2차·저항 1차·2차
[C. 추세&지표] 이평선 배열·RSI·Stochastic 해석
[D. 옵션 수급] Max Pain vs 현재가 괴리 방향·만기별 PCR 추이 해석·수급이 시사하는 단기 자석 가격대
